...
Version 0 is still the ordinary Kaspa transaction layout. It has inputs, outputs, `lock_time`, `subnetwork_id`, `gas`, `payload`, and a `storage_mass` commitment.
...
V1 defines the transaction id, full transaction hash, and signing hash as follows.
...
flowchart TB
    ...
    TX["v1 transaction<br/>inputs, outputs, lock_time,<br/>subnetwork_id, gas, payload, storage_mass"]:::tx
    ...
    ID --> IDR["BLAKE3 payload/rest split<br/>no signatures, no storage_mass,<br/>no compute_budget"]
    HASH --> HR["full TransactionHash<br/>includes signatures, storage_mass,<br/>compute_budget"]
...
- **`tx::hash`** commits to the full encoded transaction for block-level commitments, including `storage_mass` and v1 `compute_budget`.
...
For payloads, the v1 txid uses the BLAKE3 `PayloadDigest`. The sighash uses the legacy signing payload hash path.
...
| v0 `sig_op_count`                   | not allowed in v1     | not allowed in v1 | not allowed in v1            |
...
| transaction `storage_mass`          | no                    | yes               | no                           |

## Implementation notes

- V1 transactions use v1 txid construction; proofs and indexes should point at that transaction id.
- `compute_budget` is used by the transaction composer to declare execution resource usage, but it is not frozen by the signature.
- Covenant bindings let consensus check the backlink: this output is either authorized by a covenant-labeled input or is a valid genesis. The spent script must check the logical forward link: whether it really authorizes this logical transition.
- User lanes provide L1 ordering and proof anchors for based-app activity.
