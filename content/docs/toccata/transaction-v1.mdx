...
The trap is to treat execution budget like something the spender should sign forever.

That was the v0 mistake. A v0 sighash commits to sig-op counts. V1 corrects the shape: `compute_budget` is a resource commitment, not the user's covenant policy.
...
- If you expect signatures to freeze `compute_budget`, your transaction builder will fight the design.
- If you attach a covenant binding but the successor script never validates covenant state, you created a label, not a protocol.
