...
- `init_count` and `max_step` are constructor arguments. They are compile-time values for this contract instance: once the script instance is built, they are constants baked into the redeem script.
- `count` is a state field. Its current value is encoded in the redeem-script preimage, and a new value means a new P2SH commitment.
...
Silverscript covenant declarations are designed to hide the repetitive part. The compiler lowers the `Counter` transition above into an ordinary entrypoint plus a policy function. The generated helper names are shown here in readable form:

```js
contract Counter(int init_count, int max_step) {
    int count = init_count;

    function covenant_policy_increment(State prev_state, int step) : (State) {
        require(step > 0);
        require(step <= max_step);
        return({ count: prev_state.count + step });
    }

    entrypoint function increment(int step) {
        int auth_out_count = OpAuthOutputCount(this.activeInputIndex);

        State prev_state = { count: count };
        (State cov_new_state) = covenant_policy_increment(prev_state, step);
...
The wrapper does the transaction work around the policy:

- `count` is the current state value pushed by the redeem-script preimage;
- `covenant_policy_increment` is the user-written transition body, moved into a helper;
- `entrypoint function increment(int step)` is the generated spend entrypoint selected by the transaction;
- `OpAuthOutputCount` and `OpAuthOutputIdx` select the output authorized by this input;
- `require(auth_out_count == 1)` enforces the singleton transition;
- `validateOutputState(cov_out_idx, cov_new_state)` checks the successor output.

That last call uses a reflection-like awareness of the current script. The spent script uses its own revealed bytes as the template for the successor. The compiler knows the layout of the contract currently being spent. It emits code that takes the current redeem script preimage, replaces the encoded state fields with `cov_new_state`, rebuilds the expected P2SH script public key, and compares it to the authorized output.

So the output is not merely "some covenant output with the same id." It must commit to this same contract template, with the state fields changed exactly as the transition computed.
...
That changing P2SH commitment is useful: it is how the current state is committed. But it is a poor stable name for the application instance. Wallets, explorers, indexers, and other contracts need a handle that survives state transitions.

There is a second, separate issue. A transaction must not be able to mint a new UTXO that merely claims to be part of an existing covenant family. Script-only covenant designs often handle this by carrying parent or grandparent transaction data and proving the relationship inside the script. That can make forged successors unspendable later, but it does not give consensus a native rule for rejecting the forged covenant-labeled output at creation time.

KIP-20 gives Toccata that native rule.
...
Consensus enforces covenant membership at output creation:
...
Each count/index pair acts like an iterator over the relevant subset: first ask how many entries exist, then ask for the `k`-th input or output index.
