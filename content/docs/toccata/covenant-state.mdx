...
Covenants are easiest to understand if you watch one UTXO move.

Suppose you want a counter. The policy is almost boring: only increase the number, and never increase it by more than `max_step`.
...
The P2SH hash commits to both contract and state. That is exactly what you want for state validation, but it creates a naming problem.
...
- the covenant input must verify the next state, output shape, amounts, and bindings.
...
Consensus blocks unauthorized entrance into a covenant family. The script blocks invalid movement inside that family.

This is also why a covenant id on an ordinary P2PK-style output is not magic. If the spending script only checks a signature, then it only checks a signature. Covenant behavior appears when the script actually reads covenant data and validates covenant outputs.
...
Here is the shape this enables:
...
This flexibility matters. A covenant script should not have to assume that the entire transaction belongs to it. It should be able to find the subset that matters and ignore the rest.
...
Most covenant transitions are built from a small set of shapes.
...
This is the counter, vault, or single-lane state machine shape. It is simple, but it is sequential.
...
This is a recurring Toccata pattern because it keeps the expensive reasoning in one place while preventing passive participants from being dragged into the wrong transition.
...
If your app is one lane or many independent lanes, continue with [Silverscript](./silverscript).
