...
The user operation is deliberately boring at consensus level.
...
That split is the point. Kaspa L1 supplies ordering, data availability, and lane commitments. The app supplies execution semantics.
...
The important design choice is that proofs do not sit on the user-operation critical path. Execution can commit app state, while proof workers produce receipts, batches, aggregates, and settlement artifacts behind it.
...
This toy guest increments each touched resource. The pattern matters more than the logic: the host feeds encoded inputs, the guest updates resources, and the journal becomes material for later proof composition.
