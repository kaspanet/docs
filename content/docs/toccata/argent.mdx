...
It is a way to imagine the next layer above Silverscript: write a multi-contract covenant system as actors, roles, routes, and typed transitions; lower it into explicit Silverscript and then Kaspa script.

```text
Argent-shaped source -> generated Silverscript -> Kaspa script
```

The analogy is `async` lowering into a state machine. The developer writes the compact source shape. The compiler expands it into the mechanical runtime shape: state fields, templates, routing checks, leader/delegate entrypoints, and successor validation.
...
Silverscript can express multi-contract covenant systems. The hard part is not expressiveness. The hard part is keeping the route plumbing correct.
...
The Stones shape is not one contract. It is a family.
...
That shape leaves room for ordinary fee/change inputs and multiple auth groups in the same transaction.
...
- declared covenant input shape;
- declared authorized output shape;
...
Use [Decision Guide](./decision-guide) when deciding whether a multi-contract covenant family is still the right shape, or whether the app has crossed into based-app territory.
