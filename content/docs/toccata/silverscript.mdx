...
Writing raw covenant script is possible. It is also a good way to spend your week proving that a stack machine has opinions.
...
That is not just nicer syntax. It changes the review surface. Reviewers can ask "which outputs are validated?" rather than reverse-engineering raw stack choreography.
...
Generated covenant wrappers should be boring. Boring is good here. It means the same leader/delegate and output-validation patterns show up the same way across contracts.
...
That last branch is the one to sit with. A covenant can own something as a covenant, not only through a key. That is where UTXO-native composition starts to feel different from account-style composition.
