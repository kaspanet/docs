...
Ask the state-shape question first:
...
The diagram is a heuristic: choose the state shape before choosing infrastructure.
...
Kaspa's 10 BPS target matters here. If an app can partition its state, the DAG gives those lanes room to move. A covenant family that would bottleneck on a slower chain may be practical here.
