---
title: Agent Brief
description: "A compact brief for agent-assisted Toccata development, with the mistakes agents are most likely to make."
---
...
- A user-lane subnetwork ID is `[namespace: 4 bytes, 0x00 * 16]`; `[x, 0x00 * 19]` forms are reserved system IDs except native and coinbase.
- KIP-21 lanes let app proofs focus on lane activity instead of total DAG activity. Do not use the word "lane" to describe parallel UTXOs.
...
| Reaches for based apps too early                      | L1 covenants can scale when state splits into parallel UTXOs |
...
- Is the app state one UTXO, many parallel UTXOs, or one off-chain state root?
