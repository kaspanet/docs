...
Toccata makes Kaspa UTXOs expressive enough to carry real application state.

As of June 30, 2026, Toccata is active on mainnet. In `rusty-kaspa`, mainnet activation is configured at DAA score `474_165_565`.

Start with one stateful coin rather than an opcode list.
...
The rest of Toccata supports that loop at production scale: transaction fields that bind covenant outputs, opcodes that let scripts inspect the transaction they are inside, consensus lineage for covenant families, pricing that meters expensive work, and proof hooks for applications that should not execute fully on L1.

> [!IMPORTANT]
> The consensus features described here are mainnet features. The developer tooling is younger. Silverscript is the main covenant authoring path and is approaching the shape builders should expect, but release and audit status still matter. Argent and vprogs are earlier reference stacks. Expect periodic guide updates as those repos move.
...
Silverscript is the intended authoring surface for L1 covenant apps. Argent is best understood as the next layer up: actor-style, multi-contract source that can lower into Silverscript the way `async` lowers into a more mechanical runtime shape.
...
The book is arranged as a builder sequence rather than a protocol spec.
